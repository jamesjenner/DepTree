...
document.addEventListener('keyup', function(e) {
  if(e.key === "Escape") {
    ...
    for (const modal of modals) {
      // TODO: fix this, not calling the cancel event for the modal (still in add mode)
      modal.click();
    }
  }
});
...
/*
 * addNode(data, cancelAction, callback) - setup form when adding a node
 *
...
function addNode(data, cancelAction, callback) {
  ...
  // setup the dialog
  // TODO: not disabling edit mode when x button on dialog is selected
  ...
  btnAddNodeCancel.onclick = cancelAction.bind(
    this,
    callback
  );
}
...
/*
 * editNode(data, cancelAction, callback) - setup form when editing a node
 *
 *   data - the data for the node to be edited
 *   cancelAction - function called if cancel is selected
 *   callback - function called when processing is complete (I think)
...
// TODO: ESC should close the instruction. closing the instruction should cancel the add node/edge mode
...
function showInfoToast(text, listener) {
  var infoToast = document.getElementById("infoToast");
  
  var infoToastCloseBtn = document.getElementById("infoToastClose");
  var infoToastText = document.getElementById("infoToastText");
  
  infoToast.onclick = function() {
...
function setupListeners() {
  ...
  btnAddNode.onclick = function () {
    showInfoToast("Select where to add the point");
    network.addNodeMode();
  }
  ...
  btnAddLink.onclick = function () {
    showInfoToast("Drag between nodes to add link");
    network.addEdgeMode();    
  }
