...
class Network {
  ...
  getChildItems(id) {
    ...
    if(node != null && node.hasChildren()) {
      for(var i = 0; i < node.childrenIds.length; i++) {
        children.push(this.#nodes[node.childrenIds[i]].item);
      }
    }
  ...
  removeItem(itemId, updateUI = true) {
    ...
    // remove children
    for(var i = 0; i < this.#nodes[itemId].childrenIds.length; i++) {
      // delete the children for current child
      this.removeItem(this.#nodes[itemId].childrenIds[i], updateUI);
    }


    // TODO: don't really need to do this for the whole tree, just the parent one that is being removed...
    ...
    // remove from the UI, if we need to
    if(updateUI) {
      this.#removeNodeUI(itemId);
    }
  }
  ...
  #processNodeForUI(node) {
    ...
    var childrenNodes = this.getChildNodes(node)
  ...
  #addNodeToUI(node) {
    ...
    itemGroup.addEventListener("click", this.#nodeClick.bind(this));
  ...
  #updateNodeUI(item) {
    ...
    var itemGroup = document.getElementById(item.id);
    var largeGroup = itemGroup.parentNode;

    var connector = largeGroup.previousSibling;
    var ellipse = largeGroup.querySelector("ellipse");
    var mainIcon = itemGroup.children[1];
    var textName = itemGroup.nextSibling;
   ...
   *
   * id      the id of the node to be removed from the UI
   */
  #removeNodeUI(id) {
    // remove the line
    this.#container.getElementById(id).parentNode.previousSibling.remove();

    // remove the container for the item
    this.#container.getElementById(id).parentNode.remove();
  }
  ...
  #mouseEnter (event) {
    ...
    if(this.#editMode) {
      var modifyIcons = event.target.querySelectorAll(".modifyIcon");
      var iconType = "add";
      // "remove"
      // "link_off"
      // "clear"
      ...
      var children = this.getChildItems(event.target.id);
      
      // make all visible and to add mode
      event.target.querySelectorAll(".modifyIcon").forEach(x => { 
        // check if a child exists for this point
        x.style.visibility = "visible";
        x.textContent = "add";
      });

      // check children, change to remove if a child exists
      children.forEach(x => {
        switch(x.position) {
          case Item.POSITION_LEFT:
            modifyIcons[0].textContent = "remove";
            break;

          case Item.POSITION_CENTER:
            modifyIcons[1].textContent = "remove";
            break;

          case Item.POSITION_RIGHT:
            modifyIcons[2].textContent = "remove";
            break;
        }
      });
    }
  ...
  #nodeClick(event) {
    if(this.#editMode) {
      if(this.#options.editFunction != null) this.#options.editFunction(event.target.parentNode.id);
    } else {
      if(this.#options.viewFunction != null) this.#options.viewFunction(event.target.parentNode.id);
    }
  }

  #nodeEditIconClick(direction, event) {
    var itemId = event.target.parentElement.parentElement.id; // item id for the item clicked

    // check if action was add or remove
